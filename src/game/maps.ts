...
function buildPotting(): Arena {
  ...
  y.box(0, -0.4, 0, W, 0.8, D, y.m.soil, { minimap: "#D2B07A", cast: false });
  y.box(0, 0.04, 0, 14, 0.1, 10, y.m.straw, { collide: false, cast: false });

  y.box(0, H / 2, -D / 2, W, H, wall, y.m.cedar, { minimap: "#F3E4C4" });
  y.box(0, H / 2, D / 2, W, H, wall, y.m.cedar, { minimap: "#F3E4C4" });
  y.box(-W / 2, H / 2, 0, wall, H, D, y.m.cedar, { minimap: "#F3E4C4" });
  y.box(W / 2, H / 2, 0, wall, H, D, y.m.cedar, { minimap: "#F3E4C4" });
  y.box(0, 1.05, -D / 2 + 0.42, W - 2, 2.1, 0.22, y.m.moss, { collide: false });
  y.box(0, 1.05, D / 2 - 0.42, W - 2, 2.1, 0.22, y.m.moss, { collide: false });
  y.box(-W / 2 + 0.42, 1.05, 0, 0.22, 2.1, D - 2, y.m.moss, { collide: false });
  y.box(W / 2 - 0.42, 1.05, 0, 0.22, 2.1, D - 2, y.m.moss, { collide: false });
  ...
  y.box(18, 4.35, -4, 12, 0.35, 20, y.m.night, { minimap: "#8A4E24" });
  ...
  y.lamp(-12, 7.4, -6, 5.5, POTTING.lamp);
  y.lamp(6, 7.4, 4, 5.2, POTTING.lamp);
  y.lamp(18, 7.6, -8, 4.4, POTTING.lamp);
  y.lamp(-18, 7.2, 10, 4.8, POTTING.lamp);

  const hemi = new THREE.HemisphereLight(POTTING.fillSky, POTTING.fillGround, 0.85);
  y.group.add(hemi);
  y.lights.push(hemi);
  const sun = new THREE.DirectionalLight(POTTING.key, 1.15);
...
function buildCistern(): Arena {
  ...
  y.box(0, -0.45, 0, 64, 0.9, 52, y.m.soil, { minimap: "#4A5568", cast: false });
  ...
  y.lamp(-16, 6.4, -8, 3.6, CISTERN.lamp);
  y.lamp(2, 7.6, 10, 3.8, CISTERN.lamp);
  y.lamp(16, 8.4, -6, 3.4, CISTERN.lamp);

  const hemi = new THREE.HemisphereLight(CISTERN.fillSky, CISTERN.fillGround, 0.62);
  y.group.add(hemi);
  y.lights.push(hemi);
  const moonL = new THREE.DirectionalLight(CISTERN.key, 1.2);
...
function shed(
  ...
  y.box(x, h * 0.5, z, sx, h, sz, y.m.cedar, { minimap: "#243044" });
  y.box(x, h + 0.16, z, sx + 1.1, 0.32, sz + 1.1, y.m.tin, { minimap: "#8FD4DE" });
  const lip = 0.42;
